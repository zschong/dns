...
void dns_query_set_qname(dns_query_t *query, const char *domain_name) {
    ...
    // 分配内存
    query->qname = (unsigned char *)malloc(qname_len);
    if (query->qname == NULL) {
        return;
    }
    ...
    for (int i = 0; i < len; i++) {
        ...
        query->qname[pos++] = label_len; // 设置标签长度
        memcpy(query->qname + pos, domain_name + start, label_len); // 复制标签
        pos += label_len;
    }
...
char* dns_query_get_qname(const dns_query_t *query) {
    ...
    // 计算解码后的域名长度
    int len = 0;
    for (int i = 0; query->qname[i] != 0; ) {
        int label_len = query->qname[i];
    ...
    for (int i = 0; query->qname[i] != 0; ) {
        int label_len = query->qname[i];
        memcpy(domain_name + pos, query->qname + i + 1, label_len); // 复制标签
        pos += label_len;
