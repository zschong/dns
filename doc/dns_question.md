...
// 函数声明
void            dns_question_set_qname(dns_question_t *query, const char *domainName);
void            dns_question_set_qtype(dns_question_t *query, uint16_t qtype);
void            dns_question_set_qclass(dns_question_t *query, uint16_t qclass);
const uint8_t*  dns_question_get_qname(const dns_question_t *query);
uint16_t        dns_question_get_qtype(const dns_question_t *query);
uint16_t        dns_question_get_qclass(const dns_question_t *query);
void            dns_question_print(const dns_question_t *query);
...
int main() {
    ...
    dns_question_set_qname(&query, "www.example.com");
    dns_question_set_qtype(&query, 0x0001); // A记录
    dns_question_set_qclass(&query, 0x0001); // IN类

    // 打印DNS查询问题
    dns_question_print(&query);
